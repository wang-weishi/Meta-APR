...
def main():
    ...
    # Load model from check point for high->low resource finetune
    if args.load_model_dir != 'None':
        ...
        logger.info("Backbone model from {}".format(args.model_name_or_path))
            ...
            if args.do_eval:
                ...
                if args.do_eval_bleu:
                    ...
                    if dev_em > best_em:
                        ...
                        # Save best checkpoint for best em
    ...
    if args.do_test:
        logger.info("  " + "***** Testing *****")
        logger.info("  Batch size = %d", args.eval_batch_size)
        if args.tag_suffix != 'zeroshot':
            logger.info('finetuned: ' + args.task)
            for criteria in ['best-bleu', 'last']:  # 'best-ppl',
            ...
            logger.info('zeroshot eval or reproduce: ' + args.task)
            criteria = args.tag_suffix
            file = os.path.join(args.output_dir, criteria)
