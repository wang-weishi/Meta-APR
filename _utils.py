...
def convert_examples_to_features(item):
    ...
    if args.model_type == 'unixcoder':
        source_tokens = tokenizer.tokenize(source_str)[:args.max_source_length - 5]
    ...
    if args.model_type == 'unixcoder':
        pass
        # print('unixcoder: skip eos_token_id check')
    else:
...
def read_tfix_examples(filename, data_num):
    """Read examples from filename."""
    examples = []

    data = pickle5.load(open(filename, 'rb'))
    for idx, d in enumerate(data):
        assert len(d.split('<sep>')) == 2
        examples.append(
            Example(
                idx=idx,
                source=d.split('<sep>')[0],
                target=d.split('<sep>')[1]
            )
        )
        if idx + 1 == data_num:
            break
